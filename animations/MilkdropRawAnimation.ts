...
const getAnimateFunc: GetFuncType = ({ ctx, analyser, canvas }) => {
  ...
  const start = () => {
    ...
    ctx.globalAlpha = 0.08 + highs * 0.08;
    ctx.fillStyle = "black";
    ...
    for (let layer = 0; layer < 3; layer++) {
      ...
      ctx.closePath();
      ctx.strokeStyle = clrUtils.getHSL(hue + layer * 60, 100, 50 + layer * 8);
    ...
    for (let i = 0; i < pulseCount; i++) {
      ...
      ctx.fillStyle = clrUtils.getHSL(hue + i * 18, 100, 55);
      ctx.globalAlpha = 0.18 + level * 0.45;
      ctx.fill();
    }
    ...
    gradient.addColorStop(0, `hsla(${hue}, 100%, 55%, ${0.12 + bass * 0.18})`);
    gradient.addColorStop(0.45, `hsla(${hue + 90}, 100%, 40%, 0.04)`);
